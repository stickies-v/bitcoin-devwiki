...
- `analyzepsbt` examines a PSBT and provides information about what
  ...
  input, invluding whether a UTXO needs to be provided, what pubkeys
...
- `utxoupdatepsbt` searches the set of Unspent Transaction Outputs
  ...
  the signing algorithm requires information from the UTXO being.
  For segwit inputs, only the UTXO itself is necessary. However for
  ...
  Unfortunately because UTXO set only contains UTXOs and not full
