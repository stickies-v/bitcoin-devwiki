...
- The [output script descriptor](https://github.com/bitcoin/bitcoin/blob/master/doc/descriptors.md)
  ...
  (TODO pieter: mention new descriptor RPCs deriveaddresses/getdescriptorinfo, and descriptor support in importmulti)
...
(TODO pieter: it feels like this section can be merged with the earlier RPC changes section)
...
- Descriptors with key origin information imported through `importmulti` will
  ...
  PSBTs.
  (TODO pieter: this should probably be merged with the text on importmulti gaining descriptor support)
...
(TODO pieter: mention getdata randomization from #14897 and perhaps orphan tx handling from #14626)
...
(TODO pieter: mention HWI project?)
...
- This release changes the Random Number Generator (RNG) used from
  ...
  (TODO pieter: mention that the rdseed instruction is used where available)
...
### Miscellaneous
- #13961 util: Replace `boost::signals2` with `std::function` (MarcoFalke)
- #13935 contrib: Adjust output to current test format (AkioNak)
- #13862 utils: Drop `boost::interprocess::file_lock` (ken2812221)
...
- #14209 logging: Replace logprint macros with regular functions (MarcoFalke)
- #14272 init: Remove deprecated args from hidden args (MarcoFalke)
- #14494 Error if # is used in rpcpassword in conf (MeshCollider)
- #13815 util: Add [[nodiscard]] to all {decode,parse}\[…\]\(…\) functions returning bool (practicalswift)
...
- #15139 util: Remove \[u\]\(begin|end\) macros (laanwj)
...
- #15373 Move ParseConfirmTarget from rpc/mining to rpc/util (ryanofsky)
