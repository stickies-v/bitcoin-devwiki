...
_For feedback on this guide, please visit [#27736](https://github.com/bitcoin/bitcoin/issues/27736)_
...
* Test non-witness transaction that are greater than 65 bytes are allowed in the mempool and relayed [#26265](https://github.com/bitcoin/bitcoin/pull/26265)
...
Note that if you open another terminal you have to 
### set up environment variable again
```
export DATA_DIR=/tmp/25-rc-test
```
From the root of your release candidate directory, run

```
export BINARY_PATH=$(pwd)/src
```
Or else for the downloaded binary, start from the root of the downloaded release candidate (cd ~/bitcoin-25.0rc2, for example) and run:
```
export BINARY_PATH=$(pwd)/bin
```
The bcli command will be available after you run
```
alias bcli="$(echo $BINARY_PATH)/bitcoin-cli -datadir=$DATA_DIR"
```
...
```shell 
cd $DATA_DIR && echo "signet=1" >> bitcoin.conf && echo "maxconnections=0" >> bitcoin.conf && echo "dnsseed=1" >> bitcoin.conf  && echo "listen=1" >> bitcoin.conf && cd ~
```
```shell
bitcoind-test
```
If you observe the logs you will see
```
...
[Set environmental variables in the new terminal](#set-up-environment-variable-again)
...
bcli stop
```
## Test non-witness transactions that are greater than 65 bytes are allowed in the mempool
[Clear the data directory](#4-reset-testing-environment)
...
Create a transaction with an OP_RETURN output with the hex value of `6a00000000`
```
...
Add `signet=1` in the bitcoin.conf to use the signet for testing, `rest=1` to enable the rest interface.
...
## Test RPC `scanblocks` [#23549](https://github.com/bitcoin/bitcoin/pull/23549) and fast wallet rescan [#25957](https://github.com/bitcoin/bitcoin/pull/25957) with blockfiltering enabled node `-blockfilterindex=1` [#23549](https://github.com/bitcoin/bitcoin/pull/23549) and [#25957](https://github.com/bitcoin/bitcoin/pull/25957)

Block filtering [BIP 157](https://github.com/bitcoin/bips/blob/master/bip-0157.mediawiki) has been on [bitcoin core for a while](https://github.com/bitcoin/bitcoin/pull/16442) The RPC `scanblocks` utilizes block filters to Return relevant blockhashes for given descriptors. Note that for this RPC to work you have to enable block filtering.
...
```
cd $DATA_DIR && echo "regtest=1" >> bitcoin.conf && echo "blockfilterindex=1" >> bitcoin.conf && cd ~
```
`regtest=1` to use local regtest mode.
`blockfilterindex=1` to enable block filtering.

Spin up the node and create a new test-wallet
```
bitcoind-test -daemon
```
...
bcli getnewaddress
```
Get a new address to getnerate blocks.
```
bcrt1quxctp7znsvv6jjd2e44aezne8ee0ghqt4e2qsn
```
```
bcli generatetoaddress 5 bcrt1quxctp7znsvv6jjd2e44aezne8ee0ghqt4e2qsn
```
Run the RPC with the address descriptor to get all the blockhashes relevant to the address descriptor, you can create an address descriptor using `addr(address)`
```
bcli scanblocks start '["addr(bcrt1quxctp7znsvv6jjd2e44aezne8ee0ghqt4e2qsn)"]'
```
```
{
  "from_height": 0,
  "to_height": 5,
  "relevant_blocks": [
    "65ce70dd4cd23f88c0540cf6207b7a9be30f5b14a9d5cfd9bd39eece2bb87c92",
    "543c559d151ac2c56180dd25c5a26671be912adaa8da276aa23f92c17a2630b7",
    "521e4a04e412e4981840ca3351d7e83256815360615fb88b23b4f5e492e87379",
    "7f33270924feea1e8d488c6e5ef38c7caec6f10d9c91f41a4dc133a9b0cec1b6",
    "4310c488820af09bdcfca51271962425920fe2b6b4ab89b014a201d63fb3b0f2"
  ],
  "completed": true
}
```
...
bcli stop
```
### Test fast wallet rescan with a blockfiltering enabled node.

This feature also utilizes Block filtering [BIP 157](https://github.com/bitcoin/bips/blob/master/bip-0157.mediawiki) to rescan wallet almost 10x faster.
...
In the terminal run the following commands:
```
cd $DATA_DIR && echo "signet=1" >> bitcoin.conf && echo "blockfilterindex=1" >> bitcoin.conf && cd ~
```
`signet=1` to use local regtest mode.
`blockfilterindex=1` to enable block filtering.
...
```
bcli createwallet test-wallet true
```
...
bcli -getinfo
```
When you are sure it finished syncing.

Use this descriptor "addr(tb1pwzv7fv35yl7ypwj8w7al2t8apd6yf4568cs772qjwper74xqc99sk8x7tk)" has some UTXOs on the signet chain.

I calculated the checksum to get `addr(tb1pwzv7fv35yl7ypwj8w7al2t8apd6yf4568cs772qjwper74xqc99sk8x7tk)#30t3uj6k`

Try Importing the descriptor
```
bcli importdescriptors '[{"desc": "addr(tb1pwzv7fv35yl7ypwj8w7al2t8apd6yf4568cs772qjwper74xqc99sk8x7tk)#30t3uj6k","timestamp": "now","active":false,"internal": true,"next": 0}]'
```
...
```
{
  "mine": {
    ...
    "immature": 900.00004272
  },
  "lastprocessedblock": {
    "hash": "000000dd1025507bd8150b1d313ad29ec91d49b468fbb2be44fd3fe75f28ae32",
    "height": 144834
  }
}
```
Note: The balance might be higher due to the address being active as the signet network's coinbase reward address.

Try getting the UTXOs
...
[
  {
    "txid": "45bce09956d4b297c4ae48530df9bbc8416b5637bc4ba95df51995c71e9e2d9a",
    "vout": 0,
    "address": "tb1pwzv7fv35yl7ypwj8w7al2t8apd6yf4568cs772qjwper74xqc99sk8x7tk",
    "scriptPubKey": "51207099e4b23427fc40ba4777bbf52cfd0b7444d69a3e21ef281270723f54c0c14b",
    "amount": 50.00000765,
    "confirmations": 1,
    ..........
```
...
Thanks for your contribution and for taking the time to make Bitcoin awesome. For feedback on this guide, please visit [#27736](https://github.com/bitcoin/bitcoin/issues/27736)