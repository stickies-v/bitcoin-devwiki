...
```shell
abubakarsadiq@pop-os:~$ mkdir $DATA_DIR
abubakarsadiq@pop-os:~$ bitcoind-test -regtest
...
To test non-witness size, you can use a legacy Pay to public key hash address.

For convienience
```shell
$ new_address=$(bcli -regtest getnewaddress "" legacy)
```
...
This spending condition consists of an `"or"` condition and an `"and"` condition.

The `"or"` condition allows spending with either a public key derived from the extended private key "`tprv8ZgxMBicQKsPerQj6m35no46amfKQdjY7AhLnmatHYXs8S4MTgeZYkWAn4edSGwwL3vkSiiGqSZQrmy5D3P5gBoqgvYP2fCUpBwbKTMTAkL`" or a public key derived from the extended private key "`tprv8iF7W37EHnVEtDr9EFeyFjQJFL6SfGby2AnZ2vQARxTQHQXy9tdzZvBBVp8a19e5vXhskczLkJ1AZjqgScqWL4FpmXVp8LLjiorcrFK63Sr`".

The "and" condition allows spending with a script that contains both a public key derived from the extended private key "`tprv8ZgxMBicQKsPd3cbrKjE5GKKJLDEidhtzSSmPVtSPyoHQGL2LZw49yt9foZsN9BeiC5VqRaESUSDV2PS9w7zAVBSK6EQH3CZW9sMKxSKDwD`" and a time lock of `10` blocks.
