...
to 0.15.0 when that has completed.
...
- SHA256 hashing has been optimized for architectures supporting SSE 4. SHA256 is around
  ...
  validation. In v0.15, SHA256 hardware optimization is disabled in release builds by
...
Bitcoin Core v0.15.0 contains the following changes to the RPC interface and bitcoin-cli for multi-wallet:
...
Note that while multi-wallet is now fully supported, the RPC multi-wallet interface should be considered unstable for v0.15.0, and there may backwards-incompatible changes in future versions.
