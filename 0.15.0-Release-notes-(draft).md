...
- The chainstate database (which is used for tracking UTXOs) has been changed
  ...
  The only downside is that the on-disk database is 15% larger. During the conversion,
...
Changes to the estimatesmartfee and estimaterawfee APIs
-------------------------------------------------------
...
The `estimaterawfee` RPC interface has changed:

- The `nblocks` argument has been renamed to `conf_target` (to
  be consistent with other RPC methods).
- The optional `horizon` argument has been removed.
- The return value is now a JSON object containing three sub-objects,
  one for each of the `short`, `medium` and `long` time horizons. Each
  of these sub-objects is the same format as was previously returned by
  `estimaterawfee`.
- The error behavior has been changed:
    - If the `conf_target` is outside the supported range, an
    `RPC_INVALID_PARAMETER` error is raised with message `Invalid
    conf_target`
    - When an answer can't be returned due to no fee rate meeting the
    threshold at the target, the pass bucket is omitted but the fail bucket
    is returned as it contains useful information. The time horizon sub-object
    will also contain an error field reporting `Insufficient data or no
    feerate found which meets threshold`
    - When there is insufficient data to calculate whether the target is met,
    the pass bucket is omitted but fail bucket is returned as it still
    contains useful information. The time-horizon sub-object will also
    contain an error field reporting `Insufficient data or no feerate found
    which meets threshold`.
    - When the target is not supported at a time horizon, the sub-object
    for that time horizon is omitted.
    - If all feerates pass at a time-horizon, the fail bucket is omitted in
    the sub-object for that time horizon.
