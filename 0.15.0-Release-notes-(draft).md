...
The first time you run 0.15.0, your chainstate database will be converted to a
...
- The chainstate database (which is used for tracking UTXOs) has been changed
  ...
  The only downside is that the on-disk database is 15% larger.
...
since version 0.12.0, and since v0.14.0 has included a `bumpfee` RPC method to
...
In previous versions of Bitcoin Core, a portion of each block could be reserved for transactions based on the age and value of UTXOs they spent. This concept (Coin Age Priority) is a policy choice by miners, and there are no consensus rules around the inclusion of Coin Age Priority transactions in blocks. In practice, only a few miners continue to use Coin Age Priority for transaction selection in blocks. Bitcoin Core v0.15 removes all remaining support for Coin Age Priority. This has the following implications:
