...
- Estimates can now be *conservative* or *economic*. *Conservative* estimates use longer time horizons to produce an estimate which is less susceptible to rapid changes in fee conditions. *Economic* estimates use shorter time horizons and will be more affected by short-term changes in fee conditions. Economic estimates may be considerably lower during periods of low transaction activity (for example over weekends), but may result in transactions being unconfirmed if prevailing fees increase rapidly.
...
- The `estimatesmartfee` RPC interface has been changed (See [PR 10707](https://github.com/bitcoin/bitcoin/pull/10707)):
    ...
    - If Bitcoin Core has not been running for long enough and has not seen enough blocks or transactions to produce an accurate fee estimation, an error will be returned (previously a negative value `feerate` was returned).
...
- `estimaterawfee` returns raw fee data so that customized logic can be implemented to analyze the data and calculate estimates. See [Fee Estimation Improvements](https://github.com/bitcoin-core/bitcoin-devwiki/wiki/0.15.0-Release-notes#fee-estimation-improvements) for full details on changes to the fee estimation logic and interface.
...
- `listwallets` lists wallets which are currently loaded. See the *Multi-wallet* section
  of these release notes for full details (See [PR 10604](https://github.com/bitcoin/bitcoin/pull/10604)).
...
- The new database model no longer stores information about transaction
  versions of unspent outputs (See [PR 10195](https://github.com/bitcoin/bitcoin/pull/10195)). This means that:
...
- The `estimatefee` RPC is deprecated. Clients should switch to using the `estimatesmartfee` RPC, which returns better fee estimates. See [Fee Estimation Improvements](https://github.com/bitcoin-core/bitcoin-devwiki/wiki/0.15.0-Release-notes#fee-estimation-improvements) for full details on changes to the fee estimation logic and interface.
...
[Make sure that "Kibbled Jive Elk Zoo" is credited https://github.com/bitcoin/bitcoin/pull/10177#issuecomment-309244097]
[Make sure that the contributors to the release notes wiki page are credited]
