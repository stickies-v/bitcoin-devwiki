...
If you are testing on signet, you can use test coins for the tests in this guide. After IBD has completed (this process is relatively quick on signet), you can get some free corn from one of the signet faucets available.

If you already have a faucet you're familiar with using, generate a receive address and use it to get coins. Alternatively, there is a script in [`contrib/signet`][contrib/signet] that you can use to get test coins. To use, generate a new address, `cd` into the release candidate directory and run the following script:
```shell
./contrib/signet/getcoins.py -c $BINARY_PATH -a <your_receive_address>
```
...
```
Wait for your transaction to be confirmed and you will have test coins in your wallet ([watch the signet coins flowing in](https://mempool.space/signet))
...
[contrib/signet]: https://github.com/bitcoin/bitcoin/tree/master/contrib/signet
