...
- A new descriptor type `sortedmulti(...)` has been added to support multisig scripts where the public keys are sorted lexicographically in the resulting script.  (#17056)
...
- The `totalFee` argument in RPC `bumpfee`, which was deprecated in 0.19, has been removed.
  (#18312)

- `getpeerinfo` now includes a `mapped_as` field to indicate the mapped Autonomous System used for diversifying peer selection. See the `-asmap` configuration option in _New Settings_, below.
  (#16702)
...
- BIP70 support has been fully removed from Bitcoin Core (#17165). The
...
- A new `-asmap` configuration option has been added to enable IP-to-ASN mapping
  ...
  help` for more information. This option is experimental and subject to change
  or removal in future releases. (#16702)
...
- The way that output trust was computed has been fixed, which impacts confirmed/unconfirmed balance status and coin selection.  (#16766)
...
- RPC `getaddressinfo` changes:
  ...
  - the `label` field has been deprecated in favor of the `labels` field and
    ...
    with `-deprecatedrpc=label`. (#17585)
...
- The `bitcoin-cli` utility will no longer report `protocolversion` as well as `walletversion` and `keypoololdest` in the `-getinfo` aggregate response.  (#17650)
