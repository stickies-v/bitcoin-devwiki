...
- In the Peers window, the details for a peer now displays a `Mapped AS` field to indicate the mapped Autonomous System used for diversifying peer selection. See the `-asmap` configuration option in _New Settings_, above.
...
- Transactions which do not produce change outputs can still be created even if a
change address cannot be fetched.
...
- The `bitcoin-cli` utility will no longer report `protocolversion` as well as `walletversion` and `keypoololdest` in the `-getinfo` aggregate response.
