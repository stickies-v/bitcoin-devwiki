*Before the final release, move the notes here back to the git repository.*
...
- Taproot descriptors (for signet and regtest, not mainnet) can be imported into the wallet before activation in November. Spending a Taproot descriptor is also supported assuming the key path or script path is only a single public key. Spending Taproot descriptors using generic scripts, multisig schemes (k-of-n, k>1) and MuSig(2) are not yet supported.
