...
Bitcoin Core 0.16.0 introduces full support for Segwit in the wallet and user interfaces. A new `-addresstype` argument has been added, which supports legacy, p2sh, and bech32 addresses. It controls what kind of addresses are produced by getnewaddress, getaccountaddress, and createmultisigaddress. A `-changetype` argument has also been added, with the same options, and by default equal to -addresstype, to control which kind of change is used.

A new `address_type` parameter has been added to the `getnewaddress` RPC to specify which type of address to generate.
A `change_type` argument has been added to the `fundrawtransaction` RPC to override the `-changetype` argument for specific transactions.
An `address_type` argument has been added to the `addmultisigaddress` RPC to specify which address type the new multisignature address should use.
...
Note that some RPCs do not yet support segwit addresses. Notably, `signmessage`/`verifymessage` doesn't support segwit addresses, nor does `dumpwallet`/`importwallet` and `importmulti` at this time. Support for segwit in those RPCs will continue to be added in future versions.

P2WPKH change outputs are now used by default if any destination in the transaction is a P2WPKH or P2WSH output. This is done to ensure the change output is as indistinguishable from the other outputs as possible in either case.

BIP173 (Bech32) Address support
-------------------------------
Full support for native witness addresses (BIP 173 / Bech32) has now been added. 
This includes the ability to send to BIP173 addresses (including non-v0 ones), generate these
addresses (including as default new addresses, see above) and support for analyzing these types of
address through the `validateaddress` RPC.
...
--------------------------
A new RPC `rescanblockchain` has been added to manually envoke a blockchain rescan.
...
Safemode disabled by default
----------------------------
Safemode is now disabled by default and must be manually re-enabled if you wish to use it.

BIP159 signalling support
-------------------------
Pruned nodes can now signal BIP159's NODE_NETWORK_LIMITED using service bits, in preparation for
full BIP 159 support in later versions. This does not yet include support for connecting to these pruned peers.

Changes to the regtest mode
-----------------------------
...
SHA256 ASM optimizations enabled by default
-------------------------------------------
The SHA256 hashing optimizations for architectures supporting SSE 4, leading to around 50% speedups in SHA256 on supported hardware (around 5% faster IBD and block validation), has now been enabled by default. In previous versions this was enabled using the `--enable-experimental-asm` flag when building, but is now the default and is no longer deemed experimental.

GUI changes
-----------
- Uses of "µBTC" in the GUI now use the more colloquial term "bits", specified in BIP 176.
...
- #12143 `b0d626d` Fix link for BIP-159 pull request (azuchi)
