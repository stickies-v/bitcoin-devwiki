...
New `savemempool` RPC
---------------------
A new `savemempool` RPC has been added which allows the mempool to be saved to disk at
any time to avoid it being lost due to crashes / power outages.

Safemode disabled by default
----------------------------
Safemode is now disabled by default and must be manually re-enabled if you wish to use it.

Regtest JSON-RPC port default changed
-----------------------------
