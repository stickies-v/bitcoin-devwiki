...
Avoids annotating AssertLockHeld() with any compile time attributes, leaving it pure run time. This change is orthogonal and can be combined with any of the approaches above.
...
- `AssertLockHeldUnverified` name may be [confusing](https://github.com/bitcoin/bitcoin/pull/19918#issuecomment-694486228). Since both asserts do exactly the same thing at runtime and only compile time annotations differ, different naming schemes are possible. Feel free to add suggestions below:
  ...
  | Alternate suggestion | [ASSERT_&#8203;EXCLUSIVE_&#8203;LOCK_&#8203;REQUIREMENT_&#8203;SATISFIED](https://github.com/bitcoin/bitcoin/pull/19918#issuecomment-695173118) | AssertLockHeld | |
  | Other suggestions?                                           |                                 |                                 |                |
...
#### Advantages of HA Approach

- One assert implementation instead of two to eliminate confusion about when to use different asserts

#### Disadvantages of HA Approach

- [FIXME](https://github.com/bitcoin/bitcoin/pull/19979#pullrequestreview-491991997)
